...
const getAllNotificationAlongWithTodaysNotificationGeneratedByChatGpt = catchAsync(
  async (req, res) => {
    ...
    console.log("notificationFromDbForToday 🔊", notificationFromDbForToday);

    if (notificationFromDbForToday.length > 0) {
      console.log('Todays notification already generated by chatgpt  🔊', notificationFromDbForToday.length);
      ...
      if(notificationFromDbForToday.length < 2){
          ...
          console.log("newAIGeneratedNotification 💎💎💎", newAIGeneratedNotification);
    ...
    console.log("🔊 Generate notification for today .. as no notification found for today as notificationFromDbForToday.length == ", notificationFromDbForToday.length)
          ...
          try {
            ...
            if (jsonMatch) {
              try {
                ...
                console.log("🔴🔴🔴Extracted JSON from response: jsonMatch[0] :: ", jsonMatch[0]);
                // ---------------------------------------------------------------------------------
                ...
                console.error('Failed to extract valid JSON: extractError :: 🔴🔴🔴', extractError);
                jsonResponse = {
