    ...
    calculateFertilityScore = async (userId: string) => {
        try{
            ...
            if(!user.personalize_Journey_Id){
                throw new ApiError(StatusCodes.NOT_FOUND, 'Personalize Journey is not found');
            }

            const personalizeJourney = await PersonalizeJourney.findById(user.personalize_Journey_Id);
            ...
            // Get latest lab tests if available
            const labTests = await LabTestLog.find({
            where: { labTestLog_id: userId },
            order: [['date', 'DESC']],
            limit: 1
            });

            console.log('2️⃣ ', labTests);

            // Calculate sub-scores based on available data
            const scores = {
                ovulationSignals: personalizeJourney ? calculateOvulationScore(dailyInsights, personalizeJourney) : 0,
                intercourse:  personalizeJourney ? calculateIntercourseScore(dailyInsights, personalizeJourney) : 0,
                hormones: calculateHormoneScore(labTests),
                cycleConsistency: personalizeJourney ? calculateCycleConsistencyScore(personalizeJourney, dailyInsights) : 0,
                lifestyle: calculateLifestyleScore(dailyInsights)
            };

            console.log('3️⃣ score', scores);
            ...
            console.log('1️⃣ 0️⃣ final score', finalScore);

            // Ensure the score is within 0-100 range
            const boundedScore = Math.min(100, Math.max(0, finalScore));

            console.log('1️⃣ 1️⃣ boundedScore', boundedScore);

        }catch(error){
            console.log('Error in calculateFertilityScore fertie.service.ts: ', error);
            throw error;
        } 
    }
